...
void alloc_mbh_data(struct MBHData *mbh_data, struct UCBData *ucb_data, int procID)
{
    mbh_data->status = 0;
    ...
    memcpy(mbh_data->flags, ucb_data->flags, sizeof(struct Flags));
}
...
void setup_mbh_data(struct MBHData *mbh_data, struct UCBData *ucb_data, struct TDI *tdi_full, int procID)
{
    ...
    mbh_data->data->Tstart = ucb_data->data->t0;
