...
void run_ucb_mcmc(struct Data *data, struct Orbit *orbit, struct Flags *flags, struct Chain *chain, struct Source *inj)
{
    ...
    /* Initialize priors */
    struct Prior *prior = (struct Prior *)malloc(sizeof(struct Prior));
    ...
    struct Proposal **proposal = (struct Proposal **)malloc(UCB_PROPOSAL_NPROP*sizeof(struct Proposal*));
    ...
    struct Model **trial = (struct Model **)malloc(sizeof(struct Model*)*NC);
    struct Model **model = (struct Model **)malloc(sizeof(struct Model*)*NC);
